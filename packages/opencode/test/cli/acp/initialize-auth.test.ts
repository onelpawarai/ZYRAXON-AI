...
describe("zyraxon acp initialize/auth subprocess", () => {
  cliIt.live(
    ...
    ({ opencode }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.agentInfo?.name).toBe("ZYRAXON")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ opencode }) =>
      Effect.gen(function* () {
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.ZYRAXON_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
