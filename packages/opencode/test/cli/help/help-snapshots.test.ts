...
// `zyraxon --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `zyraxon --help` prints today;
...
describe("zyraxon CLI help-text snapshots", () => {
    ...
    ({ opencode }) =>
      Effect.gen(function* () {
          ...
          (argv) =>
            Effect.gen(function* () {
              ...
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`zyraxon ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`zyraxon ${argv.join(" ")} --help`)
        }
