...
it.instance("plan agent denies edits except .zyraxon/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".zyraxon/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".zyraxon", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.ZYRAXON_TEST_HOME
      process.env.ZYRAXON_TEST_HOME = test.directory
