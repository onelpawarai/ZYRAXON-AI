...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.ZYRAXON_TEST ?? "")'), {
      env: {
        ZYRAXON_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "ZYRAXON_TEST_SHELL"], {
      shell: true,
      env: {
        ZYRAXON_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("ZYRAXON_TEST_SHELL=ok")
  })
