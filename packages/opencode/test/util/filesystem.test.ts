...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "zyraxon.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "zyraxon.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "zyraxon.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "zyraxon.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["zyraxon.json", "zyraxon.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "zyraxon.json"),
        path.join(project, "zyraxon.jsonc"),
      ])
    })
  })
