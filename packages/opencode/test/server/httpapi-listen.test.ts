...
const original = {
  ...
  envPassword: process.env.ZYRAXON_SERVER_PASSWORD,
  envUsername: process.env.ZYRAXON_SERVER_USERNAME,
}
const auth = { username: "zyraxon", password: "listen-secret" }
...
afterEach(async () => {
  ...
  if (original.envPassword === undefined) delete process.env.ZYRAXON_SERVER_PASSWORD
  else process.env.ZYRAXON_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.ZYRAXON_SERVER_USERNAME
  else process.env.ZYRAXON_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.ZYRAXON_SERVER_PASSWORD = auth.password
  Flag.ZYRAXON_SERVER_USERNAME = auth.username
  process.env.ZYRAXON_SERVER_PASSWORD = auth.password
  process.env.ZYRAXON_SERVER_USERNAME = auth.username
...
describe("HttpApi Server.listen", () => {
  ...
  test("plugin client requests reuse the listening server instance", async () => {
    ...
    const previous = process.env.ZYRAXON_DISABLE_DEFAULT_PLUGINS
    process.env.ZYRAXON_DISABLE_DEFAULT_PLUGINS = "1"
    ...
    try {
      ...
      if (previous === undefined) delete process.env.ZYRAXON_DISABLE_DEFAULT_PLUGINS
      else process.env.ZYRAXON_DISABLE_DEFAULT_PLUGINS = previous
    }
  })
