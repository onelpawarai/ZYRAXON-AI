      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(
                path.join(tmp.path, "zyraxon.json"),
                JSON.stringify({ $schema: "base", providers: { base: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "zyraxon.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(documents[0]?.path).toBe(path.join(tmp.path, "zyraxon.json"))
            expect(documents[1]?.info.providers?.last).toBeInstanceOf(ConfigProvider.Info)

            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "zyraxon.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(path.join(tmp.path, "zyraxon.json"), JSON.stringify({ $schema: "base" })),
              fs.writeFile(path.join(tmp.path, "zyraxon.jsonc"), "{ invalid"),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
      ...
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".zyraxon"), { recursive: true })
            await fs.mkdir(path.join(directory, ".zyraxon"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "zyraxon.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "zyraxon.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "zyraxon.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "zyraxon.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "zyraxon.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".zyraxon", "zyraxon.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".zyraxon", "zyraxon.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".zyraxon")),
              AbsolutePath.make(path.join(directory, ".zyraxon")),
            ])
