...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.ZYRAXON_MODELS_PATH,
          disabled: Flag.ZYRAXON_DISABLE_MODELS_FETCH,
        }
        Flag.ZYRAXON_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.ZYRAXON_MODELS_PATH = previous.path
          Flag.ZYRAXON_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
